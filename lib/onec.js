...
// ---- Разбор .cf/.cfe в XML+BSL (платформенный формат) через временную базу ----
async function unpackCfToXml(exe, cfPath, outDir, ext, onData) {
  if (!exe || !fs.existsSync(exe)) return { code: -1, output: '1cv8.exe не найден' };
  if (!cfPath || !fs.existsSync(cfPath)) { if (onData) onData(`Файл не найден: ${cfPath}\n`, 'stderr'); return { code: -1, output: 'нет файла' }; }
  try { fs.mkdirSync(outDir, { recursive: true }); } catch (_) {}
  const tmpBase = path.join(os.tmpdir(), `onec-unpack-${Date.now()}`);
  const fileBase = { kind: 'file', path: tmpBase, user: '', pass: '' };
  if (onData) onData(`Разбор .cf в XML+BSL через временную базу:\n  ${cfPath}\n   → ${outDir}\n`, 'cmd');
  let code = await createInfobase(exe, tmpBase, onData);
  if (code !== 0) { cleanupDir(tmpBase); return { code, output: 'create failed' }; }
  const loadOp = ['/LoadCfg', cfPath]; if (ext) loadOp.push('-Extension', ext);
  let r = await designer(exe, fileBase, loadOp, onData);
  if (r.code !== 0) { cleanupDir(tmpBase); return r; }
  const dumpOp = ['/DumpConfigToFiles', outDir]; if (ext) dumpOp.push('-Extension', ext);
  r = await designer(exe, fileBase, dumpOp, onData);
  cleanupDir(tmpBase);
  if (onData) onData(r.code === 0 ? `✓ Готово: ${outDir}\n` : 'Ошибка разбора\n', r.code === 0 ? 'ok' : 'stderr');
  return r;
}
...
module.exports = {
  ...
  startBase, probeLock, dumpIB, pushExtension, buildCfFromXml, validateXml, unpackCfToXml,
};
