...
async function probeLock(exe, base, onData) {
  ...
  const log = readOutLog(logPath);
  try { fs.unlinkSync(logPath); } catch (_) {}
  const locked = /(блокировк|заблокирован|занят|уже открыт|уже работает|работает конфигуратор|используется|монопольн|exclusive|locked|being used|database is locked)/i.test(log);
  // попробуем вытащить, кто держит базу
  let holder = '';
  const m = log.match(/пользователь:\s*([^,]+),\s*сеанс:\s*(\d+)[^,]*,[^,]*,\s*приложение:\s*([^\r\n.]+)/i);
  if (m) holder = ` Держит: ${m[1].trim()} (сеанс ${m[2]}, ${m[3].trim()}).`;
  const msg = locked
    ? `⚠ База ЗАНЯТА — монопольный доступ невозможен, закройте Конфигуратор/сеансы.${holder}`
    : '✓ База свободна для операций (монопольный доступ доступен).';
  if (onData) { if (log) onData(log + '\n', locked ? 'stderr' : 'stdout'); onData(msg + '\n', locked ? 'stderr' : 'ok'); }
  return { code: locked ? 1 : 0, output: msg };
}
