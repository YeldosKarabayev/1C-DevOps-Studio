...
async function probeLock(exe, base, onData) {
  ...
  const log = readOutLog(logPath);
  try { fs.unlinkSync(logPath); } catch (_) {}
  const locked = /(заблокирован|занят|монопольн|exclusive|locked|используется другим)/i.test(log);
  const msg = locked
    ? '⚠ База ЗАНЯТА (монопольный доступ невозможен — закройте Конфигуратор/сеансы).'
